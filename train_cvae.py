from src.vae.cvae_dataset import CVAEDataset
from src.vae.train import train_cvae
...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, default='cvae_config.ini', help='Path to config file')
    ...
    # init model
    model = CVAE(fp_size=input_size, latent_size=latent_size).to(device)

    # train model
    _ = train_cvae(config, model, train_loader, val_loader)
