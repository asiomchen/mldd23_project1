...
returns: float, highest tanimoto score when compared with
                all the molecules from thre training set.
if returnSmiles:
...
def closest_in_train(mol, returnSmiles=False):
    datapath = 'data/GRU_data/train_morgan_512bits.parquet'
    high_tan = 0
    high_idx = 0
    train_morgan_fps = pd.read_parquet(datapath).fps.apply(eval).tolist()
    ...
    for idx, fp in enumerate(train_morgan_fps):
        fp = src.utils.finger.dense2sparse(fp, fp_len=512)
        fp = fp.tolist()
...
def get_smiles_from_train(idx):
    datapath = 'data/GRU_data/train_dataset.parquet'
    train = pd.read_parquet(datapath).smiles
