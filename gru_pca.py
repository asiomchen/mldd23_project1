...
def transform(mu_values, pca_model):
    """
    Transforms the latent space values using PCA.
    Args:
        mu_values (np.ndarray): numpy array of means of the latent space
        pca_model (sklearn.decomposition.PCA): PCA model to be used for transformation
    Returns:
        pca_results (np.ndarray): numpy array containing the transformed latent space values
    """
